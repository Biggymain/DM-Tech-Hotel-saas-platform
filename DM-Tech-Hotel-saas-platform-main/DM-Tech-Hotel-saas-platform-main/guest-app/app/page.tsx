...
export default function GuestLandingPage() {
  ...
  const [isGuestPort, setIsGuestPort] = React.useState(false);
  ...
  React.useEffect(() => {
    ...
    // ── Port 3005: Group Website & Booking Template ──────────────────────────
    if (port === '3005') {
      api.get('/api/v1/booking/group-website')
        .then((res: any) => {
          const slug = res.data?.slug;
          if (slug) {
            router.replace(`/group/${slug}`);
          } else {
    ...
    // ── Port 3004: In-Hotel Guest Portal (QR Context & Room Claim) ───────────
    ...
    if (tenant) portStorage.setItem('tenant_id', tenant);
    if (branch) portStorage.setItem('branch_id', branch);
    if (room) portStorage.setItem('room_id', room);
    if (outlet) portStorage.setItem('outlet_id', outlet);
    if (table) portStorage.setItem('table_number', table);

    // If QR signature is present, initiate the secure session handshake
    if (signature) {
    ...
    const fetchRecs = async () => {
      try {
        ...
        // Defaulting to outlet 1 for recommendations
  ...
  if (!isMounted || (!isGuestPort && typeof window !== 'undefined' && window.location.port === '3005')) {
    return (
      <div className="min-h-screen flex items-center justify-center text-white">
        <div className="flex flex-col items-center gap-3">
          <div className="h-8 w-8 rounded-full border-2 border-primary border-t-transparent animate-spin" />
          <p className="text-xs text-white/40 animate-pulse">Loading Guest Portal...</p>
        </div>
      </div>
    );
  }
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-2">
              <Input 
                ...
                value={sessionToken}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSessionToken(e.target.value.toUpperCase())}
            ...
            <Button 
              onClick={() => {
                if (sessionToken) {
                  router.push(`/menu?booking_id=${encodeURIComponent(sessionToken)}`);
                } else {
                  router.push('/menu');
                }
              }}
