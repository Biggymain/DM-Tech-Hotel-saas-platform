...
export function POSMobileView({ activeWaitressId, isTerminalMode = false }: POSMobilePageProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (typeof window === 'undefined' || !(window as any).Echo || !effectiveUserId || !user?.hotel_id) return;

    const ch = (window as any).Echo.private(`hotel.${user.hotel_id}.waiter.${effectiveUserId}`)
