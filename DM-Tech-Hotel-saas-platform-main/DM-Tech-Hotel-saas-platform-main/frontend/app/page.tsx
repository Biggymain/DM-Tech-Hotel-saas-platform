...
export default function LandingPage() {
  ...
  const [isGroupPort, setIsGroupPort] = React.useState(false);

  // ── Port-Aware Dispatcher ──────────────────────────────────────────────────
  useEffect(() => {
    setIsMounted(true);
    ...
    // Port 3000 (Master Control / SIEM):
    if (port === '3000') {
      if (user) {
        router.replace(user.is_super_admin ? '/master-control' : redirectPath || '/organization');
      } else {
    ...
    // Port 3002 (Property GM & Receptionist):
    if (port === '3002') {
      if (user) {
    ...
    // Port 3003 (Staff Operations / POS / KDS):
    if (port === '3003') {
      if (user) {
        router.replace(redirectPath || '/kds');
      } else {
        router.replace('/staff/pin');
      }
      return;
    }
  }, [isLoading, user, redirectPath, router]);

  if (!isMounted || (!isGroupPort && typeof window !== 'undefined' && window.location.port !== '3001')) {
    return (
      <div className="min-h-screen bg-[#07090e] flex items-center justify-center text-white">
        <div className="flex flex-col items-center gap-3">
          <div className="h-8 w-8 rounded-full border-2 border-primary border-t-transparent animate-spin" />
          <p className="text-xs text-white/40 animate-pulse">Routing to Port Entrypoint...</p>
        </div>
      </div>
    );
  }
