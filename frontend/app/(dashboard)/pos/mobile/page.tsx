...
export default function POSMobilePage() {
  ...
  useEffect(() => {
    if (typeof window === 'undefined' || !window.Echo || !user?.id || !user?.hotel_id) return;

    const ch = window.Echo.private(`hotel.${user.hotel_id}.waiter.${user.id}`)
  ...
  const fireOrder = useMutation({
    mutationFn: (orderId: number) => api.post(`/api/v1/pos/orders/${orderId}/fire`),
    onSuccess: () => {
  ...
  const handleFireOrder = async () => {
    ...
    if (draftOrderId) {
      ...
      // Create draft then immediately fire it
      const { data } = await api.post('/api/v1/pos/orders', {
        outlet_id: user?.outlet_id,
        table_number: tableNo,
        items: cart.map((i) => ({ menu_item_id: i.id, quantity: i.qty, unit_price: i.price, notes: i.notes })),
      });
    ...
    <div className="min-h-screen bg-background pb-44 max-w-lg mx-auto">
      ...
      <div className="sticky top-0 z-30 border-b bg-card px-4 py-3 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-2">
          {/* Incoming ready-order notification badge */}
          {readyNotifs.length > 0 && (
            ...
            >
              ...
              {readyNotifs.length} order{readyNotifs.length > 1 ? 's' : ''} ready!
            </button>
          )}
          <input
            value={tableNo}
            ...
            className="w-28 border rounded-lg px-2 py-1.5 text-sm bg-background text-center"
          />
          {cartQty > 0 && (
      ...
      {cart.length > 0 && (
        <div className={`fixed bottom-0 left-0 right-0 z-40 max-w-lg mx-auto bg-card border-t shadow-2xl transition-all ${cartOpen ? 'rounded-t-2xl' : ''}`}>
          ...
          <div className="p-4 space-y-2">
            ...
            {/* The "Fire Order" button — central feature */}
            <Button
            ...
            {/* Save as draft for review first */}
            {!draftOrderId && (
              <button
                onClick={() => createDraft.mutate()}
                disabled={createDraft.isPending || cart.length === 0 || !tableNo}
                className="w-full h-9 text-xs font-semibold rounded-xl border border-border text-muted-foreground hover:bg-muted transition-all disabled:opacity-40"
              >
                Save as Draft (review before firing)
              </button>
            )}
            {draftOrderId && (
              <p className="text-center text-xs text-emerald-600 font-medium">
                <CheckCircle2 className="inline h-3.5 w-3.5 mr-1" />Draft #{draftOrderId} saved — tap 🔥 Fire to send
              </p>
            )}

            {!tableNo && <p className="text-center text-xs text-muted-foreground">⚠️ Set table/room number first</p>}
          </div>
        </div>
      )}
    </div>
